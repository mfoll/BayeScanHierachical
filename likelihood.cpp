...
double allelecount_loglikelihood()
{
    ...
    #pragma omp parallel for SCHED_I reduction(+:loglikelihood) private(g, phi_sc)
    for (int i=0;i<I;i++)
    {
        if (!discarded_loci[i])
        {
            ...
            for (int j=0;j<J;j++)
            {
                ...
                {
                    phi_sc=exp(-(eta2[pop[j].group][i]+theta[j]));
                    loglikelihood+=factln(pop[j].locus[i].alleleCount)+gammaln(phi_sc)
                                   -gammaln(pop[j].locus[i].alleleCount+phi_sc);
                    for (int k=0;k<pop[j].locus[i].ar;k++)
                        loglikelihood+=gammaln(pop[j].locus[i].data_allele_count[k]+phi_sc*group[pop[j].group].locus[i].allele[k])
                                       -factln(pop[j].locus[i].data_allele_count[k])-gammaln(phi_sc*group[pop[j].group].locus[i].allele[k]);
                }
                else // intensity
                {
